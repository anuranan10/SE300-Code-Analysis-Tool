...
public class KeyWord_Processor
{
    ...
    private List<string> keyLines = new List<string>();


    private bool termInArray(string term, List<string> target)
    {
    ...
    private string executeParseTermsLoop()
    {
        ...
        for (int i = 0; i < charFile.Length; i++)
        {
                ...
                { 
                    ...
                    switch (charFile[i])
                    {
                        case '£':
                            ...
                            break;
                        case '¢':
                            var userMadeClassesList = Globals.userMadeClasses.Cast<string>().ToList();
                            if (termInArray(currentTerm.ToLower(), userMadeClassesList))
                            {
                                keyLines.Add(currentTerm);
                                currentTerm = "";
                            }
            ...
            else
            {
                ...
                if (termInArray(currentTerm, fieldKWS.ToList()) ) //if the current term is found inside fieldKWS array
                {
                    switch (currentTerm)
                    {
                        case "/*":
